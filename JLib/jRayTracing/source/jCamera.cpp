...
jCamera::jCamera(const jVec3f &eye_pos, const jVec3f &look_at, int width, int height)
{
    ...
    _position = eye_pos;
    _direction = (look_at - _position).normalize_org();

    J_ALIGN(16) jVec3f jVec3f_0_0_1(0, 0, 1);

    _x_direction = jVec3f_0_0_1.cross_org(_direction * jReal(-1)).normalize_org();
    ...
    _x_spacing_half = _x_spacing * jReal(0.5);
    _y_spacing_half = _y_spacing * jReal(0.5);
}
