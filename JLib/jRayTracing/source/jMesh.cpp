...
jMesh::jMesh(const jVec3f &pos, const char* file_path, jIMaterial* material) : jSceneObj(pos), _materialPtr(material)
{
    ...
    for (auto i = 0; i < materials_size; i++) {
        ...
        if (!_materials[i].diffuse_texname.empty()) {
            ...
            //_matPtrs.push_back(new jRefractMaterial(_materialPtr->get_color()));
        }
        else {
            //_matPtrs.push_back(new jRefractMaterial(_materialPtr->get_color()));
        }
    ...
    for (auto i = 0; i < shapes_size; i++) {
        ...
        for (size_t f = 0; f < indices_size; f++) {
            ...
            jTriangle* aligned_buffer = (jTriangle*)_aligned_malloc(sizeof(jTriangle), 16);

            //if (_shapes[i].mesh.material_ids[f] < _matPtrs.size())
            //    tris.push_back(new jTriangle(v0_, v1_, v2_, t0_, t1_, t2_, _matPtrs[_shapes[i].mesh.material_ids[f]]));
            //else
                tris.push_back(new (aligned_buffer) jTriangle(v0_, v1_, v2_, t0_, t1_, t2_, _materialPtr));
        }
    }
...
jRayObjectItersection jMesh::get_intersection(const jRay& ray)
{
    ...
    jReal t = 0, tmin = std::numeric_limits<jReal>::infinity();
